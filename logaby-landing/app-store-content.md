# Logaby - App Store Connect Content

## App Name (30 chars max)
```
Logaby - Baby Tracker Voice
```

## Subtitle (30 chars max)
```
Log Feedings, Sleep, Diapers
```

## Keywords (100 chars max, comma-separated, no spaces)
```
baby tracker,newborn,feeding log,diaper,sleep tracker,pumping,breastfeed,nursing,nap,voice,AI
```
*(96 characters - optimized for high-volume parenting keywords)*

---

## App Description (4000 chars max)

```
TRACK YOUR BABY WITH YOUR VOICE

No more fumbling with your phone while holding a crying baby at 3am. Just tap the microphone and speak naturally—our AI understands you.

"She ate 4 ounces"
"Wet diaper just now"
"Baby is going down for a nap"

Done. Logged. Synced.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

🤖 AI-POWERED VOICE LOGGING

Logaby uses advanced AI to understand natural speech. No rigid commands or complicated menus. Just say what happened, and we'll categorize it for you.

• Feedings (bottle & breastfeeding)
• Diaper changes (wet, dirty, dry)
• Sleep & naps
• Pumping sessions
• Weight tracking

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

👨‍👩‍👧 FAMILY SYNC

Everyone stays on the same page. Mom, Dad, grandma, the nanny—all caregivers see updates in real-time. No more "did you feed her?" texts.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

📊 SHARE WITH YOUR PEDIATRICIAN

Generate beautiful PDF reports to share with your doctor. Track patterns, spot trends, and have real data for your next checkup.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

🎯 BUILT FOR EXHAUSTED PARENTS

We know the struggle. We've been up at 3am trying to remember when the last feeding was. That's why we built Logaby—the baby tracker that works when your hands are full.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

✨ KEY FEATURES

• AI voice recognition - just speak naturally
• Track feedings, diapers, sleep, pumping
• Family sync - share with caregivers
• PDF reports for pediatricians
• Beautiful dashboard with daily summaries
• Works offline - sync when you're back online

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

💰 SIMPLE PRICING

Try everything FREE for 7 days. Then:
• $19.99 for 6 months — the "First Months Pass"

That's just $3.33/month for peace of mind during the most critical time.

No auto-renewal. No surprises.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Download Logaby today and finally have a free hand.
```

---

## Promotional Text (170 chars max)
```
🆕 NEW: AI-powered voice logging! Just tap the mic and speak naturally. Try FREE for 7 days.
```

---

## What's New (for updates)
```
• NEW: AI-powered voice logging - just speak naturally!
• Improved sleep tracking
• Family sync now even faster
• Bug fixes and performance improvements

Thanks for using Logaby! Leave a review if you love it ❤️
```

---

## URLs Needed
- Support: https://logaby.com/support.html ✅
- Marketing: https://logaby.com ✅
- Privacy: https://logaby.com/privacy.html ✅

---

## Screenshots Needed (6)

| # | Screen | Caption |
|---|--------|---------|
| 1 | Voice input with waveform | "Just tap and speak" |
| 2 | Today's dashboard | "Your day at a glance" |
| 3 | AI parsing demo | "AI understands you" |
| 4 | Family sync | "Keep everyone in sync" |
| 5 | PDF report | "Share with your doctor" |
| 6 | Activity history | "Track every moment" |

**Devices:** iPhone 6.7", 6.5", 5.5"

---

## Category
- Primary: Health & Fitness
- Secondary: Lifestyle

## Age Rating: 4+

---

## Keyword Strategy Notes

**High-volume keywords targeted:**
- "baby tracker" - highest volume
- "newborn" - high intent new parents
- "feeding log" - specific feature search
- "sleep tracker" - parents searching for sleep help
- "breastfeed" / "nursing" - lactating mothers
- "pumping" - breast pump users
- "voice" / "AI" - differentiator from competitors

**Our differentiator:**
Logaby is the ONLY baby tracker with AI voice input. This is our key selling point.
